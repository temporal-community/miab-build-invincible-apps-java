# Meetup in a Box - Build Invincible Apps with Durable Execution - Java
...
This presentation is designed to introduce Java developers of all experience levels to Temporal.
...
This repository contains the sample application for the _Build Invincible Apps with Durable Execution - Java_ presentation for Meetup in a box.
For more information about this presentation, visit the presentations [landing page](https://learn.temporal.io/meetup_in_a_box/invincible_apps/java/).

## Installation

In order to present this material, you will need to install a development version of the Temporal Service, the Temporal Java SDK, and various packages onto your presentation device.

1. Install the Temporal CLI on your local machine by following [this tutorial](https://learn.temporal.io/getting_started/java/dev_environment/#set-up-a-local-temporal-service-for-development-with-temporal-cli)
    1. This presentation uses [Maven](https://maven.apache.org/) to manage dependencies and build applications. You are welcome to use whatever tool you like, but all instructions here will be Maven.
2. Clone this repository to your local machine:
    ```bash
    git clone https://github.com/temporal-community/miab-build-invincible-apps-java.git
    ```
3. Change directories into the cloned repository:
    ```bash
    cd miab-build-invincible-apps-java
    ```
4. As you change directories into the various demos, be sure to compile the code prior to running. This will install the dependencies as well:
    ```bash
    mvn clean compile
    ```
...
The Temporal Service you started will not be used in this portion.
    ...
    3. Show the code to the audience, demonstrating it's a standard loop. Then run the Normal Execution script
        ```bash
        mvn exec:java -Dexec.mainClass="normal.Main" 
        ```
    ...
    7. Return to the top level of the git repository for the next part of the demo
        ```bash
   ...
   **Remember**, this is not the time to go deep into the architecture of Temporal.
   ...
   Mention that for this sample, there's a running process that executes the code and another script that invokes it. 
    ...
    2. Compile the code:
        ...
        mvn clean compile
        ```
    3. Show the files in `durable/src/main/java/durable` to the audience. 
        Explain that it's a slight modification, but relatively similar.
        You added a method to do the addition and put it into its own Interface and implementation.
        It is also a process that runs and is invoked by a starter script.
        Everything was abstracted out into an Interface and implementation pattern.
        **DON'T** go into depth here, all the terms will be defined later.
    3. Start the Temporal Worker:
        ```bash
        mvn exec:java -Dexec.mainClass="durable.DurableWorker"
        ```
        ...
        [INFO]
        [INFO] ------------< io.temporal.learn:durable-execution-practice >------------
        [INFO] Building durable execution (practice) 1.0.0-SNAPSHOT
        [INFO]   from pom.xml
        [INFO] --------------------------------[ jar ]---------------------------------
        [INFO]
        [INFO] --- exec:3.5.0:java (default-cli) @ durable-execution-practice ---
        10:01:37.244 INFO  - Created WorkflowServiceStubs for channel: ManagedChannelOrphanWrapper{delegate=ManagedChannelImpl{logId=1, target=127.0.0.1:7233}}
        ```
    4. Change to another terminal and run the starter script:
        ```bash
        mvn exec:java -Dexec.mainClass="durable.Starter"
        ```
        This code will **not** return immediately. Return to the other terminal.        
...
If not, run the following command to start the Temporal Service on port 8080 with a persistent database:
    ```bash
    temporal server start-dev --ui-port 8080 --db-filename temporal.db
    ```
    You should see an output _similar_ to this (the exact output and versions may differ)
